...
class May29 {//open Main

    //variable declaration and initialization
    //instance variables accessible to all methods
    private static Scanner uI = new Scanner(System.in);
    private static String name = null;
    private static boolean JackOfDiamonds = false, KingOfClubs = false, QueenOfHearts = false, AceOfSpades = false;
    ...
    public static void main (String [] args){//open main()
        ...
        System.out.print("\nEnter your name: ");
        name = uI.next();//does not assure username as it can be numbers, letters or a combination of both

        System.out.printf("\nWelcome to Wonderland %s.\n", name);

        do {
            if (lives==0){//game ends when the user has 0 lives left
           ...
           userChoice = assureGuess(uI, userChoice);//assigns user input to userChoice variable and assures input with assureGuess method
            ...
            case 1:
                if (!alreadyHave(AceOfSpades, "Ace of Spades")){
            ...
            case 2:
            if (!alreadyHave(JackOfDiamonds, "Jack of Diamonds")){
            ...
            case 3:
            if (!alreadyHave(KingOfClubs,"King of Clubs")){
                ...
                if (!AceOfSpades || !KingOfClubs || !JackOfDiamonds) {//the player can only enter the final room once they have one the games in the other rooms and collected the respective items
                    ...
                    do { 
                            System.out.printf("Round %d: ", rounds++);//displays round number, player can win in as few as 2 rounds
...
⠀⠀⠀⠀⠀⠀⠙⠛⠿⠿⠿⠟⠋⠁⠈⠙⠛⠿⠿⠿⠿⠿⠿⠿⠿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    public static void numberGame (int range, int b, int c) { //method for number guessing game
        ...
        int x = 0;//resets for each game
        int rounds = 0;//resets for each game
        randnum = (int)(Math.random()*range+1);//generates a random number for a specified range defined in the parameter
        ...
        System.out.println(endGameMessage[b]);//prints the corresponding message for the end of the game
        x++;//increments x, and breaks out of while loop to end number guessing game once number is guessed correctly
        switch (c){//item collected depending on the room
            case 1:
                AceOfSpades = true;
                break;
            case 2:
...
`OOOO;@@@@@@@@aa@@@@@@@@@' 
  ;@@@@@@@@@@@@@@@@@@@' 
   ...
   `@@@@@'    @@@@@' 
    ...
    private static Boolean alreadyHave(Boolean a, String b){
        Boolean userPlay = false;
        if(a){//if the player has already won this round they have collected the item and will be prompted back to the menu to choose a round they have not won yet
            System.out.printf("\nYou already have the %s card! Get the other cards so you can escape Wonderland.\n", b);
            userPlay = true;
        }
        return userPlay;
    }
