/*@Author: Azka Siddiqui                        @Date: June 7, 2023
 *@File: CPTAzka
 ...
 * The result of this match will determine the users win or loss of the entire game and whether they can escape Wonderland! 
 */

 import java.util.Scanner;//import scanner class
 class cptWithRoom {//open Main
 
     //instance variables accessible to all methods
     private static Scanner uI = new Scanner(System.in);//instantiate scanner object
     private static int items [] = new int[4];//array to store collected items
    ...
    private static void Paper(){//Paper method for ASCII art
     ...
     private static int assureGuess (Scanner uI, int a){//assures user input for a number in guessing game
         ...
         a = uI.nextInt();//update
         return a;//method returns validated guess
     }
     ...
     private static void numberGame (int range, int endMessageIndex, int card) { //method for number guessing game
        ...
        while (lives > 0 && !gameOver){//Asks user for guess while the user has lives left and the game is not over (user has tries left)
        ...
        if (userguess != randnum) {//test for if player guessed incorrectly
            ...
            if ((3-rounds)==0){//asks player if they want to return to lobby once they have no more lives left
                lives--;//decrement life count
                ...
                returnLobby();//asks user if they want to return to lobby
            }
        } else {
            System.out.println(endGameMessage[endMessageIndex]);//prints the corresponding message for the end of the game
            gameOver = true;//exits number guessing game loop
            items[userChoice-1]=1;//sets items element from 0 to 1 since item has been collected, userChoice-1 is the index for the array as userChoice starts at 1 but the array elements start at 0
