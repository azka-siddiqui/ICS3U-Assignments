 ...
 */
 ...
 class CPTAzkaSiddiqui2 {//open Main
     ...
     public static void main (String [] args){//open main()
        ...
        System.out.println("\nWelcome, cheater. The correct answers to the rooms will be displayed!");
         ...
         do {
            ...
            playerChoice = 0;//resets player choice of game option at the end of each game
            options();//displays game options
 
            playerChoice = assureInput(uI, playerChoice);//assigns user input to playerChoice variable and assures input with assureGuess method
             ...
             case 1:
                ...
                numberGame(3, 0, 1, uI, items);//method for guessing game of a number 1-3, if guess is correct displays endgame message and the prize is the Ace of Spades card
                break;
             ...
             case 2:
                 ...
                 numberGame(4, 1, 2, uI, items);//method for guessing game of a number 1-4, if guess is correct displays endgame message and the prize is the Jack of Diamonds card
                 break;
 
             case 3:
                 System.out.println("\nYou find yourself in the thick of a forest, and on the way, you encounter a most mysterious cat. A grinning Cheshire cat, to be exact.  You tell the cat that you are searching for the cards you need, and ask him which way you should go.  He tells you that you could visit either the March Hare or the Queen of Hearts--it doesn't really matter which because both of them are mad.  More confused than ever, you catch the glimmer of a card on a tree bench up high beside the Cheshire Cat. He flashes a mischevious grin at you and challenges you to a game, which if you win, you will get the card he has.");
                 Cheshire();
                 numberGame(5, 2, 3, uI, items);//method for guessing game of a number 1-5, if guess is correct displays endgame message and the prize is King of Clubs card
                 break;
                 ...
                 if (itemsCollected != 3){//the player can only enter the final room once they have one the games in the other rooms and collected the respective items
                     ...
                     do { 
                             ...
                             do { 
                               ...
                               playerChoice = assureInput(uI, playerChoice);//assure user input for a number

                               if (playerChoice < 1 || playerChoice > 3){//chechs if player entered an invalid move
                                System.out.println("\nInvalid entry. Please choose an option from 1-3.");
                               }
 
                               } while (playerChoice < 1 || playerChoice > 3); //repeats code in loop prompting player for a valid move from 1-3
            ...
            default://if playerchoice does not meet the cases above, checks if player entered a valid option
                while (playerChoice < 1 || playerChoice > 4){//option chosen must be between 1 and 4, loops until playerChoice is a valid option
                System.out.print("\nInvalid choice. Please enter a number between 1 and 4: ");
                playerChoice = assureInput(uI, playerChoice);//assures input to be a number
                }
            }//close switch
        }//close if statement
    ...
    private static void Paper(){//Paper method for ASCII art
     ...
     private static int assureInput (Scanner uI, int a){//assures user input for a number in guessing game
     ...
     private static void numberGame (int range, int endMessageIndex, int card, Scanner uI, int items[]) { //method for number guessing game
        ...
        playerguess = assureInput(uI, playerguess);//assures user input for number
